...
def course_list(request):
    ...
    elif selected_audience == Course.Audience.COSMETOLOGISTS:
...
def course_detail(request, pk):
    ...
    if request.method == "POST" and form.is_valid():
        ...
        with transaction.atomic():
            ...
            duplicate_filters = Q(applicant_phone=(applicant_phone))

            if request.user.is_authenticated:
                duplicate_filters |= Q(student=request.user)
            ...
            existing_application = (
                ...
                .exclude(status=(CourseEnrollment.Status.CANCELLED))
                .order_by("-enrolled_at")
                .first()
            )

            if existing_application:
                request.session["last_course_application_id"] = (
            ...
            application = CourseEnrollment.objects.create(
                ...
                applicant_comment=(form.cleaned_data["applicant_comment"]),
                course=locked_course,
                created_by=(
                    request.user if (request.user.is_authenticated) else None
                ),
            ...
            request.session["last_course_application_id"] = application.pk

            transaction.on_commit(
                lambda application_id=(application.pk): (
                    notify_course_application_created(application_id)
                )
            )
...
def application_success(
    ...
    last_application_id = request.session.get("last_course_application_id")

    if last_application_id != application_id:
        raise Http404("Заявку не знайдено.")
